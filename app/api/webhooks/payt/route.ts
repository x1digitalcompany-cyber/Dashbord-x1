...
export async function GET() {
  return json(200, { ok: true, service: "dashboard-x1-payt", message: "Webhook Payt ativo. Envie POST com JSON." });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed.transaction_id) {
      console.warn("[webhook/payt] sem transaction_id — ignorado");
    ...
    if (parsed.status !== "approved") {
      console.warn(
        `[webhook/payt] status não reconhecido tx=${parsed.transaction_id} status=${parsed.status}`
      );
      return json(200, { received: true, ignored: "unknown_status" });
    }

    await supabase.from("payt_payments").upsert(
