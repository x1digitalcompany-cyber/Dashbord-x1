...
import { parsePaytPayment } from "@/lib/payment-parsers";
...
export async function GET() {
  return json(200, {
    ok: true,
    service: "dashboard-x1-x1company",
    message: "Webhook X1Company ativo. Envie POST com JSON.",
  });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = parsePaytPayment(rawBody);

    if (!parsed.transaction_id) {
      console.warn("[webhook/x1company] sem transaction_id — ignorado");
    ...
    if (parsed.status !== "approved") {
      console.warn(
        `[webhook/x1company] status não reconhecido tx=${parsed.transaction_id} status=${parsed.status}`
      );
      return json(200, { received: true, ignored: "unknown_status" });
    }

    await supabase.from("x1company_payments").upsert(
    ...
    console.error("[webhook/x1company]", err);
